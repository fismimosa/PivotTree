...
class RuleTree(RuleTreeBase, ABC):
    ...
    def _set_stump(self):
        ...
        # Normalize probabilities to handle floating-point precision errors
        total_p = sum(_p)
        _p = [p / total_p for p in _p]
    
        
        assert abs(sum(_p) - 1.0) < 1e-9, "Probabilities do not sum to 1."
    ...
    def fit(self, X: np.array, y: np.array = None, **kwargs):
        ...
        while len(self.queue) > 0 and nbr_curr_nodes + len(self.queue) < self.max_leaf_nodes:
           ...
           # print(name_clf)
            ...
            global_labels = labels ##HERE CHANGED
