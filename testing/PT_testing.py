...
results_folder = "datasets/testing_res"
...
for name in all_datasets:
    ...
    rt = RuleTreeClassifier(distance_measure='euclidean', base_stumps = [pt_stump, multi_pt_stump,obl_pt_stump,multi_obl_pt_stump], stump_selection = 'best', random_state = 0, 
                            max_depth = 3)
    ...
    for fold_idx, (fold_tuple, fold_matrix) in enumerate(zip(folds, folds_proximity_matrix)):
        ...
        for combination in param_combinations:
            ...
            for key, value in dict_pivs.items():
                try:
                ...
                for extra_clf_name, extra_clf in extra_classifiers.items():
                    if 'knn_' in extra_clf_name:
                        try:
                            ...
                            extra_clf.fit(X_train_fold[value], y_train_fold[value])
                            ...
                            result_entry = {
                                **params,
                                ...
                                "Fit Time (s)": fit_time,
                                "Predict Time (s)": predict_time,
                                "Additional Classifier" : f'{key}_{extra_clf_name}_feature_space',
                ...
                for extra_clf_name, extra_clf in extra_classifiers.items():
                    ...
                    # Measure fitting time
                    start_fit_extra = time.process_time()
                    extra_clf.fit(X_train_reduced, y_train_fold)
                    end_fit_extra = time.process_time()
                    fit_time_extra = end_fit_extra - start_fit_extra
    
                    start_predict_extra = time.process_time()
                    y_pred = extra_clf.predict(X_test_reduced)
                    end_predict_extra = time.process_time()
                    predict_time_extra = end_predict_extra - start_predict_extra
                    ...
                    result_entry = {
                            **params,
                            ...
                            "Fit Time (s)": fit_time,
                            "Predict Time (s)": predict_time,
                            "Additional Classifier" : f'{key}_{extra_clf_name}',
                            "Additional Classifier Fit Time (s)" : start_fit_extra,
                            "Additional Classifier Predict Time (s)" : predict_time_extra,
                            "n_discriminatives" : len(dict_pivs['discriminatives']),
                            "n_descriptives" : len(dict_pivs['descriptives']),
    ...
    results_df.to_csv(f'datasets/testing_res/mstz_{dataset_name}_results.csv', index = False)
    ...
    grouped_results.to_csv(f'datasets/testing_res/mstz_{dataset_name}_grouped.csv', index = False)
