...
os.environ['MKL_NUM_THREADS'] = f'{num_thr}'
os.environ['OPENBLAS_NUM_THREADS'] = f'{num_thr}'
os.environ['GOTO_NUM_THREADS'] = f'{num_thr}'
os.environ['OMP_NUM_THREADS'] = f'{num_thr}'
...
images = ['doctoral','MNIST', 'cifar10', 'catsdogs', 'waterbirds',
          'oxfordpets', 'organamnist', 'bloodmnist', 'SVHN']
...
already_split = images + time_series +text[:-1]

all_datasets =  tabular + time_series + text + images

all_datasets = ['doctoral', 'SVHN','yeast_new', 'algerian_forest_fires_new','iris_new','wine_new','page_blocks_binary_new','spambase_new','compas_new','magic_new','house16_new'] + all_datasets


all_datasets = ['LiarPantsOnFire','polarity_dataset_v2.0_pang_cs_cornell_embed']



results_folder = "datasets/subset_PIVOT_FOREST_DEFAULT_testing_OS_32"
...
for name in all_datasets:
   ...
   # data_pivots
    ...
    for p in [0.5, 0.10, 0.15, 0.2, 0.25 , 0.3, 0.35, 0.4, 0.45]:
    ...
    with parallel_backend('loky', n_jobs=num_thr):
        ...
        for fold_idx, (fold_tuple, fold_matrix) in enumerate(zip(folds, folds_proximity_matrix)):
            ...
            for e, combination in enumerate(param_combinations):
                ...
                rt = RuleForestClassifier(**params, n_jobs = num_thr)
                ...
                for p, clf in extra_classifiers.items():
                   ...
                   # print(data_pivots)
                    ...
                    if not len(rt.estimators_) < 1:
                        ...
                        #print(X_test_reduced.shape[1])
            
                        LR = LogisticRegression(max_iter = 1000, n_jobs = num_thr)
    ...
    results_df.to_csv(f'datasets/subset_PIVOT_FOREST_DEFAULT_testing_OS_32/mstz_{dataset_name}_results_simple.csv', index = False)
    ...
    grouped_results.to_csv(f'datasets/subset_PIVOT_FOREST_DEFAULT_testing_OS_32/mstz_{dataset_name}_grouped_simple.csv', index = False)
    ...
    grouped_results.to_csv(f'datasets/RF_testing_res/mstz_{dataset_name}_grouped.csv', index = False)
